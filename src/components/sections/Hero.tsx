...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    /* Depth layers — far → near shifts near-white to warm champagne.
       ...
       Blur multiplier drops on near layer so big radii don't produce runaway halos. */
    const LAYERS = [
      { n:70, rMin:0.12, rRange:0.55, vyMin:0.018, vyRange:0.055, alpha:0.20, blur:3.5,  wobAmp:0.09, wsScale:0.012, fill:'255,250,235', glow:'245,232,172' }, // far  — tiny whisps
      { n:50, rMin:0.45, rRange:1.40, vyMin:0.055, vyRange:0.090, alpha:0.30, blur:4.0,  wobAmp:0.21, wsScale:0.016, fill:'255,245,215', glow:'242,220,148' }, // mid  — mixed sizes
      { n:30, rMin:1.25, rRange:4.50, vyMin:0.095, vyRange:0.130, alpha:0.22, blur:3.0,  wobAmp:0.36, wsScale:0.020, fill:'255,238,192', glow:'238,208,118' }, // near — large bokeh orbs
    ];

    type Layer = typeof LAYERS[0];
    type Particle = {
      x: number; y: number; r: number; vy: number; vx: number;
      life: number; max: number; alpha: number; blur: number;
      wobAmp: number; wobble: number; ws: number;
      fill: string; glow: string; cfg: Layer; layer: number;
    };

    function mkP(cfg: Layer, layer: number, scatter: boolean): Particle {
      const w = canvas!.width || 800, h = canvas!.height || 900;
      return {
        x:      Math.random() * w,
        y:      scatter ? Math.random() * h : h * 0.5 + Math.random() * h * 0.6,
    ...
    function tick() {
      ...
      ps.forEach((p, i) => {
        ...
        p.y += p.vy;
        p.life++;
        if (p.y < -10 || p.life > p.max) { ps[i] = mkP(p.cfg, p.layer, false); return; }
        const a = Math.sin(p.life / p.max * Math.PI) * p.alpha;
        ctx!.fillStyle   = `rgba(${p.fill},${a.toFixed(3)})`;
        ctx!.shadowColor = `rgba(${p.glow},${(a * 0.72).toFixed(3)})`;
  ...
  return (
    ...
    >
      ...
      {/* ── HERO IMAGE ─────────────────────────────────────────────
          ...
          Desktop: blurred + scaled 110% to kill blur edges, 28% — tonal texture.
      ───────────────────────────────────────────────────────────── */}
      <div className="absolute inset-0 z-0">
        <Image
          ...
          className="object-cover object-top
                     opacity-90
                     md:opacity-[0.28] md:blur-2xl md:scale-110"
        />
      </div>
      ...
      {/* ── AMBIENT BOKEH ──────────────────────────────────────── */}
      <div className="absolute rounded-full pointer-events-none z-[3]" style={{ width:'220px', height:'220px', background:'radial-gradient(circle,rgba(196,112,10,.08),transparent)', top:'15%', left:'18%', filter:'blur(36px)', animation:'corePulse 6s ease-in-out infinite' }} />
      <div className="absolute rounded-full pointer-events-none z-[3]" style={{ width:'160px', height:'160px', background:'radial-gradient(circle,rgba(165,39,70,.07),transparent)', bottom:'22%', right:'14%', filter:'blur(36px)', animation:'corePulse 8.5s ease-in-out 1.8s infinite' }} />
      <div className="absolute rounded-full pointer-events-none z-[3]" style={{ width:'90px', height:'90px', background:'radial-gradient(circle,rgba(245,202,40,.18),transparent)', top:'38%', left:'38%', filter:'blur(36px)', animation:'corePulse 4.2s ease-in-out .8s infinite' }} />
      <div className="absolute rounded-full pointer-events-none z-[3]" style={{ width:'60px', height:'60px', background:'radial-gradient(circle,rgba(165,39,70,.14),transparent)', top:'28%', right:'28%', filter:'blur(36px)', animation:'corePulse 5.5s ease-in-out 3s infinite' }} />
      ...
      <div
        ...
        style={{ height: '110px', gap: '18px', background: 'linear-gradient(to top, rgba(88,18,36,.14) 0%, transparent 100%)' }}
      >
        <svg width="52" height="44" viewBox="0 0 52 44" style={{ opacity: .42, filter: 'drop-shadow(0 0 7px rgba(165,39,70,.4))' }}>
          <path d="M26 44 Q17 27 13 13 Q26 22 26 8 Q26 22 39 13 Q35 27 26 44Z" fill="rgba(148,36,64,.42)" stroke="rgba(192,72,96,.28)" strokeWidth=".5"/>
          <path d="M26 44 Q8 30 4 16 Q20 24 26 8" fill="rgba(118,28,52,.32)"/>
          <path d="M26 44 Q44 30 48 16 Q32 24 26 8" fill="rgba(118,28,52,.32)"/>
        </svg>
        <svg width="74" height="60" viewBox="0 0 74 60" style={{ opacity: .52, filter: 'drop-shadow(0 0 10px rgba(165,39,70,.48))' }}>
          <path d="M37 60 Q26 40 20 22 Q37 31 37 12 Q37 31 54 22 Q48 40 37 60Z" fill="rgba(165,39,70,.52)" stroke="rgba(202,78,106,.32)" strokeWidth=".5"/>
          <path d="M37 60 Q13 44 5 26 Q26 34 37 12" fill="rgba(130,30,58,.38)"/>
          <path d="M37 60 Q61 44 69 26 Q48 34 37 12" fill="rgba(130,30,58,.38)"/>
        </svg>
        <svg width="48" height="40" viewBox="0 0 48 40" style={{ opacity: .36, filter: 'drop-shadow(0 0 7px rgba(165,39,70,.3))' }}>
          <path d="M24 40 Q16 24 12 12 Q24 20 24 7 Q24 20 36 12 Q32 24 24 40Z" fill="rgba(148,36,64,.4)" stroke="rgba(192,72,96,.22)" strokeWidth=".5"/>
        </svg>
      </div>
      ...
      ───────────────────────────────────────────────────────────── */}
      <div
        className="absolute bottom-40 left-1/2 -translate-x-1/2 text-center pointer-events-none z-[11] whitespace-nowrap"
      >
        <div
          style={{
      ...
      ───────────────────────────────────────────────────────────── */}
      <button
        onClick={() => document.getElementById('featuredset')?.scrollIntoView({ behavior: 'smooth' })}
      ...
      >
        ...
        <svg
          width="96" height="96" viewBox="0 0 60 60" fill="none"
          style={{ animation: 'spinCW 18s linear infinite', opacity: .72 }}
        >
          ...
          <circle cx="30" cy="30" r="26" stroke="rgba(196,136,12,.16)" strokeWidth=".6"/>
        ...
        >
          <path d="M1 1L7 7L13 1" stroke="rgba(232,180,24,.9)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </button>
